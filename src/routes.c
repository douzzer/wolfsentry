    ...
    struct wolfsentry_route_table *route_table)
{
    ...
    if ((ret = wolfsentry_route_new(
        ...
        return ret;
    return wolfsentry_id_generate(wolfsentry, WOLFSENTRY_OBJECT_TYPE_ROUTE, &route_table->fallthrough_route->header.id);
}
    ...
    wolfsentry_action_res_t *action_results)
{
    ...
    if ((ret = wolfsentry_id_generate(wolfsentry, WOLFSENTRY_OBJECT_TYPE_ROUTE, &route_to_insert->header.id)) < 0)
        return ret;
...
{
    struct wolfsentry_eventconfig_internal *config = (rule_route->parent_event && rule_route->parent_event->config) ? rule_route->parent_event->config : &wolfsentry->config;
    wolfsentry_errcode_t ret;
    ...
    if (rule_route->parent_event && rule_route->parent_event->match_event && rule_route->parent_event->match_event->action_list.header.head) {
        WOLFSENTRY_WARN_ON_FAILURE(wolfsentry_action_list_dispatch(
                                       wolfsentry,
                                       caller_arg,
                                       rule_route->parent_event->match_event,
                                       trigger_event,
    ...
    if (rule_route->parent_event && rule_route->parent_event->decision_event && rule_route->parent_event->decision_event->action_list.header.head) {
        WOLFSENTRY_WARN_ON_FAILURE(wolfsentry_action_list_dispatch(
                                       wolfsentry,
                                       caller_arg,
                                       rule_route->parent_event->decision_event,
                                       trigger_event,
    ...
    )
{
    ...
    /* if dynamic is disabled, jump out and use default static policy. */
    ...
    if (rule_route == NULL) {
        if ((ret = wolfsentry_route_clone(wolfsentry, &target_route->header, wolfsentry, (struct wolfsentry_table_ent_header **)&rule_route, WOLFSENTRY_CLONE_FLAG_NONE)) < 0)
            goto just_free_resources;
        if (trigger_event)
            rule_route->parent_event = trigger_event;
        else
...
wolfsentry_errcode_t wolfsentry_route_format_address(
    ...
    const struct wolfsentry_route_endpoint *endpoint,
    const byte *addr,
    char *buf,
    int *buflen)
{
    ...
    if (WOLFSENTRY_ERROR_CODE_IS(
            ...
            OK))
    {
        wolfsentry_errcode_t ret = formatter(wolfsentry, addr, endpoint->addr_len, buf, buflen);
        if (ret < 0)
            return ret;
        WOLFSENTRY_RETURN_OK;
    }
    ...
    if (sa_family == WOLFSENTRY_AF_LINK) {
        int i;
        if ((endpoint->addr_len >> 3) * 3 > (size_t)*buflen)
            WOLFSENTRY_ERROR_RETURN(BUFFER_TOO_SMALL);
        for (i=0; i < (endpoint->addr_len >> 3); ++i) {
            if (i > 0)
                *buf++ = ':';
        ...
        *buflen = (int)(buf - buf_at_start);
        ...
        memcpy(addr_buf, addr, WOLFSENTRY_BITS_TO_BYTES(endpoint->addr_len));
        ...
        *buflen = (int)strlen(buf);
        ...
        if (WOLFSENTRY_BITS_TO_BYTES(endpoint->addr_len) >= (size_t)*buflen)
            WOLFSENTRY_ERROR_RETURN(BUFFER_TOO_SMALL);
        memcpy(buf, addr, WOLFSENTRY_BITS_TO_BYTES(endpoint->addr_len));
        buf[WOLFSENTRY_BITS_TO_BYTES(endpoint->addr_len)] = 0;
        *buflen = (int)WOLFSENTRY_BITS_TO_BYTES(endpoint->addr_len);
        WOLFSENTRY_RETURN_OK;
    } else
        WOLFSENTRY_ERROR_RETURN(OP_NOT_SUPP_FOR_PROTO);
}

static wolfsentry_errcode_t wolfsentry_route_render_address(struct wolfsentry_context *wolfsentry, int sa_family, int addr_bits, const byte *addr, size_t addr_bytes, FILE *f) {
...
static wolfsentry_errcode_t wolfsentry_route_render_endpoint(struct wolfsentry_context *wolfsentry, const struct wolfsentry_route *r, int sa_local_p, FILE *f) {
    ...
    if (sa_local_p ? (r->flags & WOLFSENTRY_ROUTE_FLAG_SA_LOCAL_ADDR_WILDCARD) : (r->flags & WOLFSENTRY_ROUTE_FLAG_SA_REMOTE_ADDR_WILDCARD))
        ...
        wolfsentry_errcode_t ret = wolfsentry_route_render_address(wolfsentry, r->sa_family, (int)e->addr_len, addr, addr_bytes, f);
        if (ret < 0)
            return ret;
    }
